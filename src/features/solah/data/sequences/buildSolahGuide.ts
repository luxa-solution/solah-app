...
export function buildSolahGuide(
  ...
  items.push(...withIdPrefix(solah, "r1", buildFirstRakaah(solah)));

  // Rakaah 2

  if (rakaat === 2) {
    items.push(...withIdPrefix(solah, "r2", [...buildSecondRakaah(solah), ...finalSitting]));
  } else {
    items.push(...withIdPrefix(solah, "r2", [...buildSecondRakaah(solah), ...firstTashahhud]));
  }

  // Rakaah 3

  if (rakaat === 3) {
    items.push(...withIdPrefix(solah, "r3", [...buildLaterRakaah(solah), ...finalSitting]));
  }

  // Rakaah 4

  if (rakaat === 4) {
    items.push(...withIdPrefix(solah, "r3", buildLaterRakaah(solah)));

    items.push(...withIdPrefix(solah, "r4", [...buildLaterRakaah(solah), ...finalSitting]));
  }
